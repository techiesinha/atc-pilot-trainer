...
function validateCategory(cat?: string): MetarData['flightCategory'] {
  return (['VFR','MVFR','IFR','LIFR'] as const).includes(cat as MetarData['flightCategory'])
    ? cat as MetarData['flightCategory']
    : 'UNKNOWN';
}
...
function buildInterpretation(raw: RawAWC, cat: MetarData['flightCategory']): MetarInterpretation {
  ...
  const altimeterSummary = `QNH ${Math.round(qnh)} hPa. ${
    qnh < 1000 ? 'Low pressure — density altitude elevated, performance reduced.'
